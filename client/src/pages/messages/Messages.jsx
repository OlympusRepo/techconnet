...
const Messages = () => {
  ...
  const tableActions = data?.map((item, i) => ({
    buyer: (
      <p
        className={`md:py-5 h-full text-xs font-semibold w-[90px] md:w-full md:text-sm truncate ${
          (authUser?.isTechnician && !item?.readBytechnician) ||
          (!authUser.isTechnician && !item?.readByBuyer && "!bg-slate-100")
        }`}
      >
        {authUser?.isTechnician ? item.buyerId : item.sellerId}
      </p>
    ),
    lastMessage: (
      <Link
    ...
    action: (
      ...
      >
        {((authUser.isTechnician && !item.readBytechnician) ||
          (!authUser.isTechnician && !item.readByBuyer)) && (
          <button
